...
def api_dashboard():
    ...
    cur.execute("""
        SELECT LEFT(first_contact, 7) AS month,
               COUNT(*) AS leads,
               COUNT(*) FILTER (WHERE status = 'Venta') AS sales,
               COALESCE(SUM(sale_amount) FILTER (WHERE status = 'Venta'), 0) AS amount
        FROM clients
...
def mark_manual_response(phone_number):
    """
    Respaldo manual: si Meta no entrega smb_message_echoes, permite
    marcar el momento en que una asesora/área respondió.
    Solo fija la primera respuesta si todavía está pendiente.
    """
    conn = get_db()
    cur = conn.cursor()
...
def assign_advisor(phone_number):
    ...
    allowed = {None, "Narly", "Raphaella", "Ursula", "Ecommerce", "Post Venta", "Inmobiliaria"}
