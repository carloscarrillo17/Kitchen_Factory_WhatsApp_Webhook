...
def calculate_business_response_seconds(first_contact, response_at):
    """
    Tiempo atribuible a atención dentro del horario 10:00-20:00, hora de Lima.
    Se calcula día por día y funciona también si el lead llegó fuera de horario.
    """
    start = parse_iso_lima(first_contact)
    end = parse_iso_lima(response_at)
    ...
    while day <= end_day:
        work_start = datetime(day.year, day.month, day.day, 10, 0, tzinfo=LIMA_TZ)
        work_end = datetime(day.year, day.month, day.day, 20, 0, tzinfo=LIMA_TZ)

        segment_start = max(start, work_start)
        segment_end = min(end, work_end)
...
def api_dashboard():
    ...
    # Actividad 24 horas y tiempos de respuesta.
    # Usa opportunities para contar conversaciones/leads por hora sin borrar ni modificar históricos.
    hourly_where = ["opened_at IS NOT NULL"]
    hourly_params = []
    if date_from:
        hourly_where.append("LEFT(opened_at, 10) >= %s")
        hourly_params.append(date_from)
    if date_to:
        hourly_where.append("LEFT(opened_at, 10) <= %s")
        hourly_params.append(date_to)
    hourly_where_sql = " WHERE " + " AND ".join(hourly_where)

    cur.execute(f"""
        SELECT
            EXTRACT(HOUR FROM opened_at::timestamptz AT TIME ZONE 'America/Lima')::int AS hour,
            COUNT(*) AS total
        FROM opportunities
        {hourly_where_sql}
        GROUP BY hour
        ORDER BY hour
    """, hourly_params)
    hourly_total_rows = cur.fetchall()
    hourly_totals = [0] * 24
    for row in hourly_total_rows:
        hour = int(row["hour"])
        if 0 <= hour <= 23:
            hourly_totals[hour] = int(row["total"] or 0)

    advisors = ["Ursula", "Narly", "Raphaella"]
    leads_by_advisor = {a: [0] * 24 for a in advisors}
    response_buckets = {a: [[] for _ in range(24)] for a in advisors}

    cur.execute(f"""
        SELECT advisor, opened_at
        FROM opportunities
        {hourly_where_sql}
          AND advisor = ANY(%s)
        ORDER BY opened_at
    """, hourly_params + [advisors])
    for row in cur.fetchall():
        dt = parse_iso_lima(row.get("opened_at"))
        advisor = row.get("advisor")
        if dt and advisor in leads_by_advisor:
            leads_by_advisor[advisor][dt.hour] += 1

    client_resp_where = ["advisor = ANY(%s)", "first_contact IS NOT NULL"]
    client_resp_params = [advisors]
    if date_from:
        client_resp_where.append("LEFT(first_contact, 10) >= %s")
        client_resp_params.append(date_from)
    if date_to:
        client_resp_where.append("LEFT(first_contact, 10) <= %s")
        client_resp_params.append(date_to)

    cur.execute(f"""
        SELECT advisor, first_contact, first_response_at, response_time_seconds
        FROM clients
        WHERE {" AND ".join(client_resp_where)}
    """, client_resp_params)
    advisor_rows = cur.fetchall()

    summary_map = {
        a: {
            "advisor": a, "leads": 0, "responded": 0,
            "real_values": [], "business_values": [], "outside_hours": 0
        }
        for a in advisors
    }

    # Resumen de leads por asesora se toma de opportunities para respetar conversaciones.
    for advisor in advisors:
        summary_map[advisor]["leads"] = sum(leads_by_advisor[advisor])

    for row in advisor_rows:
        advisor = row.get("advisor")
        first_dt = parse_iso_lima(row.get("first_contact"))
        if advisor not in summary_map or not first_dt:
            continue

        if first_dt.hour < 10 or first_dt.hour >= 20:
            summary_map[advisor]["outside_hours"] += 1

        if row.get("first_response_at"):
            business_seconds = calculate_business_response_seconds(
                row.get("first_contact"), row.get("first_response_at")
            )
            real_seconds = row.get("response_time_seconds")
            if real_seconds is None:
                real_seconds = calculate_response_seconds(
                    row.get("first_contact"), row.get("first_response_at")
                )

            summary_map[advisor]["responded"] += 1
            if real_seconds is not None:
                summary_map[advisor]["real_values"].append(float(real_seconds))
            if business_seconds is not None:
                summary_map[advisor]["business_values"].append(float(business_seconds))
                response_buckets[advisor][first_dt.hour].append(float(business_seconds))

    response_by_advisor = {}
    for advisor in advisors:
        response_by_advisor[advisor] = [
            (round(sum(vals) / len(vals), 1) if vals else None)
            for vals in response_buckets[advisor]
        ]

    advisor_summary = []
    all_business_values = []
    for advisor in advisors:
        s = summary_map[advisor]
        all_business_values.extend(s["business_values"])
        advisor_summary.append({
            "advisor": advisor,
            "leads": s["leads"],
            "responded": s["responded"],
            "avg_real_response_seconds": (
                round(sum(s["real_values"]) / len(s["real_values"]), 1)
                if s["real_values"] else None
            ),
            "avg_business_response_seconds": (
                round(sum(s["business_values"]) / len(s["business_values"]), 1)
                if s["business_values"] else None
            ),
            "outside_hours": s["outside_hours"]
        })

    peak_index = max(range(24), key=lambda h: hourly_totals[h]) if any(hourly_totals) else None
    peak_hour = (
        f"{peak_index:02d}:00–{(peak_index + 1) % 24:02d}:00"
        if peak_index is not None else None
    )
    outside_business_hours = sum(hourly_totals[:10]) + sum(hourly_totals[20:])
    before_open = sum(hourly_totals[:10])
    avg_business_response_seconds = (
        round(sum(all_business_values) / len(all_business_values), 1)
        if all_business_values else None
    )
    ...
    hourly_activity = {
        ...
        "avg_business_response_seconds": avg_business_response_seconds,
        "leads_by_advisor": leads_by_advisor,
        "response_by_advisor": response_by_advisor,
