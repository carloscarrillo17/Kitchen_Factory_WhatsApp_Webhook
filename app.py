...
def update_client(phone_number, contact_name, timestamp, source,
    ...
    if existing:
        lead_type, conversation_count = classify_returning_lead(
            existing.get("last_contact"),
            timestamp,
            existing.get("lead_type") or "Nuevo",
            existing.get("conversation_count") or 1
        )

        # Si Meta entrega referral, esa atribución tiene prioridad.
        # Si no lo entrega, no afirmamos que sea orgánico: queda "Sin atribución".
        final_source = existing.get("source") or "Sin atribución"
        final_campaign = existing.get("campaign_name")
        final_ad_id = existing.get("ad_id")
        final_referral = existing.get("referral_json")

        if source == "Meta Ads":
            final_source = "Meta Ads"
            final_campaign = campaign_name or final_campaign
            final_ad_id = ad_id or final_ad_id
            final_referral = referral_json or final_referral
        elif final_source not in ("Meta Ads",):
            final_source = "Sin atribución"
        ...
        """, (
            ...
            timestamp,
            1,
            source if source == "Meta Ads" else "Sin atribución",
            campaign_name,
            ad_id,
            referral_json,
            "Nuevo",
